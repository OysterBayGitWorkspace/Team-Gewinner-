...
DIM_LABELS = [("funding_market", "Funding"), ("capital_access", "Capital access"), ("regulation", "Regulation"),
              ("strategics", "Strategics"), ("exit_comps", "Exit comps")]
HEALTH_LABELS = [("runway", "Runway"), ("risk_band", "Risk band"), ("kpi_trend", "KPI trend"), ("contact", "Contact")]
...
def render_html(pulse: dict, run_report: dict, segments_cfg: dict) -> str:
    ...
    for c in companies:
        partners = "".join(
            f'<li>{link(p.get("source_url"), p["name"])} <span class="muted small">({esc(p["kind"])}, {esc(p.get("date") or "undated")}) {esc(p.get("evidence"), 160)}</span></li>'
            for p in c["partners"]) or '<li class="muted">no dated counterparties found in the segment research</li>'
        urg_cls = {"NOW": "tag-red", "QUARTER": "tag-amber", "WATCH": "tag-sand"}[c["urgency"]]
        prop_cards.append(f"""
        <div class="card prop">
          <div class="prop-head">
            <div><h3>{esc(c['company'])}</h3><div class="muted small">{esc(segments_cfg.get(c['segment'], {}).get('label', c['segment']))} · {esc(', '.join(c['funds']))}</div></div>
            <div>{tag(c['urgency'], urg_cls)} {tag('company ' + c['health'].lower())} {tag('market ' + c['market'].lower())}</div>
          </div>
          <p class="route">{esc(c['route_label'])}</p>
          ...
          <p class="small"><strong>Counterparties to work:</strong></p><ul class="small">{partners}</ul>
        </div>""")
    ...
    for c in companies:
        ...
        grid_rows.append(f"<tr><td><a href='#c-{esc(slug(c['company']))}'>{esc(c['company'])}</a></td>{cells}"
                         f"<td>{esc(c['route'])}</td><td>{esc(c['urgency'])}</td></tr>")
    ...
    for c in companies:
        ...
        <details class="card" id="c-{esc(slug(c['company']))}">
          <summary><strong>{esc(c['company'])}</strong> · {esc(c['route'])} · {esc(c['urgency'])}
            <span class="muted small">cash {esc(internal.get('cash') or 'n/a')} as of {esc(internal.get('cash_as_of') or 'n/a')} · runway {esc(internal.get('runway_months') if internal.get('runway_months') is not None else 'n/a')} m · process {esc(internal.get('active_process') or 'unknown')}</span>
          </summary>
          {seg_note}
          <div class="cols">
            <div><h4>Internal (Jarvis)</h4><ul class="small">{notes}</ul><h4>Attention items (risk rank)</h4><ul class="small">{att}</ul></div>
            <div><h4>External (last 12 months)</h4><ul class="small facts">{ext}</ul><h4>Coverage</h4><ul class="small">{cov}</ul></div>
          </div>
        </details>""")
    ...
    for sid, sc in pulse["segments"].items():
        ...
        if data is None:
            ...
            funds = "".join(f"<li>{link(f.get('source_url'), f['name'])} <span class='muted small'>{esc(f.get('date') or 'undated')} · {esc(f['evidence'], 200)}</span></li>" for f in data["capital_access"]["active_funds"]) or "<li class='muted'>none found</li>"
            players = "".join(f"<li>{link(p.get('source_url'), p['name'])} <span class='muted small'>{esc(p.get('date') or 'undated')} · {esc(p['evidence'], 200)}</span></li>" for p in data["strategics"]["players"]) or "<li class='muted'>none found</li>"
            ...
            facts = "".join(fact_li(f) for dim in ("funding_market", "regulation") for f in data[dim]["facts"])
            body = f"""<div class="cols">
              ...
              <div><h4>Exit comps (24 m)</h4><ul class="small">{deals}</ul><h4>Funding and regulation facts</h4><ul class="small facts">{facts or "<li class='muted'>none</li>"}</ul></div>
            </div>"""
...
.container{{max-width:1240px;margin:0 auto;padding:0 clamp(24px,3vw,48px)}}
...
.section{{padding:48px 0}} .section-dark{{background:var(--black);color:var(--pearl)}} .section-sand{{background:var(--sand-dry)}}
.card{{background:var(--white);border-radius:.75rem;padding:24px;margin-bottom:12px}}
.tag{{display:inline-block;padding:.25rem .75rem;border-radius:100rem;font-size:.72rem;font-weight:500;text-transform:uppercase;letter-spacing:.05em;margin-left:4px}}
.tag-dark{{background:var(--black);color:var(--pearl)}} .tag-sand{{background:var(--sand-dry);color:var(--sand-wet)}} .tag-red{{background:#c0392b;color:#fff}} .tag-amber{{background:#d9a53a;color:#000}}
.muted{{color:var(--sand-wet)}} .small{{font-size:.85rem}} .warn-text{{color:#c0392b}}
.banner{{background:var(--sand-dry);padding:12px 0;font-size:.9rem}} .banner.warn{{background:#d9a53a}}
.light{{display:inline-flex;align-items:center;gap:6px;margin-right:14px;font-size:.8rem}} .dot{{width:12px;height:12px;border-radius:50%;display:inline-block}} .lbl{{color:var(--sand-wet)}}
...
table{{width:100%;border-collapse:collapse;font-size:.85rem}} th{{text-align:left;font-weight:500;padding:8px;border-bottom:2px solid var(--black)}} td{{padding:8px;border-bottom:1px solid rgba(0,0,0,.08);vertical-align:top}} tr:hover td{{background:var(--sand-dry)}}
...
<div class="banner{banner_cls}"><div class="container">Run {esc(run_report['run_id'])} · {n_ok}/{n_all} workers succeeded · research cost {total_cost:.2f} USD · generated {esc(run_report['generated_at'])}
 · <span class="legend"><span><span class="dot" style="background:{LIGHT_HEX['green']};display:inline-block;width:10px;height:10px;border-radius:50%"></span> supportive</span><span><span class="dot" style="background:{LIGHT_HEX['yellow']};display:inline-block;width:10px;height:10px;border-radius:50%"></span> mixed</span><span><span class="dot" style="background:{LIGHT_HEX['red']};display:inline-block;width:10px;height:10px;border-radius:50%"></span> adverse</span><span><span class="dot" style="background:{LIGHT_HEX['grey']};display:inline-block;width:10px;height:10px;border-radius:50%"></span> no data</span></span></div></div>
...
<div class="section"><div class="container"><h2>Portfolio grid</h2><div class="card table-wrap"><table><thead><tr><th>Company</th>{grid_head}<th>Route</th><th>Urgency</th></tr></thead><tbody>{''.join(grid_rows)}</tbody></table></div>
<p class="small muted">First four lights are internal (Jarvis), the next five are the segment's external market. Hover a light for the rule that produced it.</p></div></div>

<div class="section section-sand"><div class="container"><h2>Companies</h2>{''.join(details)}</div></div>
...
def render_markdown(pulse: dict, run_report: dict, segments_cfg: dict) -> str:
    ...
    for c in pulse["companies"]:
        ...
        lines.append(f"- **{c['company']}** ({lab}) · {c['urgency']} · company {c['health']} / market {c['market']} · **{c['route_label']}**. Trigger: {c['urgency_reason']}.")
        if c["partners"]:
            lines.append("  - Counterparties: " + "; ".join(f"{p['name']} ({p['kind']}, {p.get('date') or 'undated'})" for p in c["partners"][:4]))
    n_ok = sum(1 for w in run_report["workers"] if w["ok"])
